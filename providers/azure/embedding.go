...
func (p *Provider) SupportEmbedding() bool {
	// Azure OpenAI supports embeddings, but we need to implement it properly.
	// For now, return false to satisfy the interface.
	return false
}

// BuildEmbeddingRequest creates an HTTP request for the Azure OpenAI Embedding API.
func (p *Provider) BuildEmbeddingRequest(ctx context.Context, req *types.EmbeddingRequest) (*http.Request, error) {
	return nil, errors.NewInvalidRequestError(ProviderName, "", "embedding not yet implemented for azure")
}

// ParseEmbeddingResponse transforms an Azure OpenAI response into the unified format.
