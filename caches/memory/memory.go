// Package memory provides an in-memory cache implementation with LRU + TTL eviction.
package memory
...
// Cache implements an in-memory cache with LRU + TTL eviction.
...
// Config holds configuration for memory Cache.
type Config struct {
...
func (c *Cache) Set(ctx context.Context, key string, value []byte, ttl time.Duration) error {
	...
	if len(value) > c.maxItemSize {
		return nil // Silently skip oversized items
	}
